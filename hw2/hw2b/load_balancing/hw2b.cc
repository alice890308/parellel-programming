...
int main(int argc, char** argv) {
    ...
    double highesttime = 0.0, lowesttime = 1000000, averagetime = 0.0;
    ...
    #pragma omp parallel
	{
        ...
        double endtime = MPI_Wtime() - tmptime;
        #pragma omp critical
        {
            highesttime = (endtime > highesttime) ? endtime : highesttime;
            lowesttime = (endtime < lowesttime) ? endtime : lowesttime;
            averagetime += endtime;
        }
    }
    averagetime = averagetime / ncpus;

    if (rank != 0) {
        // char text[20];
        // sprintf(text, "rank%d.png", rank);
        // write_png(text, iters, width, height, image);
        MPI_Reduce(&averagetime, NULL, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
        MPI_Reduce(&highesttime, NULL, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
        MPI_Reduce(&lowesttime, NULL, 1, MPI_DOUBLE, MPI_MIN, 0, MPI_COMM_WORLD);
    ...
    //printf("size = %d\n", size);
    if (rank == 0) {
        MPI_Reduce(MPI_IN_PLACE, &averagetime, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
        MPI_Reduce(MPI_IN_PLACE, &highesttime, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
        MPI_Reduce(MPI_IN_PLACE, &lowesttime, 1, MPI_DOUBLE, MPI_MIN, 0, MPI_COMM_WORLD);
        ...
        std::cout << "highest time: " << highesttime << '\n';
