...
void* calculate(void*obj) {
    ...
    double cnst_y0 = (upper - lower) / height;
    ...
    while(1) {
        ...
        for (i = 0; i < width;) {
            ...
            while (repeats_v[0] < iters && repeats_v[1] < iters && length_sqr_v[0] < 4 && length_sqr_v[1] < 4) {
                ...
                // ++repeats_v;
                y_v = _mm_add_pd(_mm_mul_pd(two_v, _mm_mul_pd(x_v, y_v)), y0_v);
                x_v = _mm_add_pd(_mm_sub_pd(x_sqr_v, y_sqr_v), x0_v);
                x_sqr_v = _mm_mul_pd(x_v, x_v);
                y_sqr_v = _mm_mul_pd(y_v, y_v);
                length_sqr_v = _mm_add_pd(x_sqr_v, y_sqr_v);
                repeats_v = _mm_add_pd(one_v, repeats_v);
            }
